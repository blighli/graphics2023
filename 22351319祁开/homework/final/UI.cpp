...
void UI::show() {
	...
	ImGui::NewFrame();
	{
		...
		//ImGui::Text(u8"坐标：%5.2f %5.2f %5.2f"u8, application->camera.Position.x, application->camera.Position.y, application->camera.Position.z);
		ImGui::Text(u8"大作业：实体追逐模拟%s"u8);
		...
		case 1:
			...
			break;
		case 2:
			ImGui::Text(u8"第三人称模式下按WASD移动主控实体 按住右键移动鼠标旋转摄像机"u8);
			break;
		default:
			break;
		}
		ImGui::Text(u8"\n备注：\n基于ecs系统模拟10k个实体追逐主控\n碰撞检测用空间格子实现，算法仍存在问题"u8);
		//if (i >= 0) {
