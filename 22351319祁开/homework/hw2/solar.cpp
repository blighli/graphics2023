...
#include <ex/stb_image.h>

#define TINYOBJLOADER_IMPLEMENTATION
#include <ex/tiny_obj_loader.h>
...
//中文字符串处理
const char* operator "" u8(const char8_t* str, std::size_t) {
	return reinterpret_cast<const char*>(str);
}
const int WIDTH = 1280;
const int HEIGHT = 720;
const std::string MODEL_PATH = "models/plant.obj";
const std::string TEXTURE_PATH = "textures/2k_earth_daymap.jpg";
const int MAX_FRAMES_IN_FLIGHT = 2;
Camera* cameraptr = nullptr;
class vulkanAPP* appptr = nullptr;
//物体
struct Object
{
	...
	glm::mat4& calculate() {
		model = glm::translate(glm::mat4(1.0f), location) * ratation * scale;
		return model;
	}
};
std::vector<Object> objects;
uint32_t select_index=-1;//点选物体的id
void initObjects() {
	Object sun;
	sun.name = u8"太阳"u8;
	sun.location = glm::vec3(0., 0, 0);
	sun.texture_path = "textures/2k_sun.jpg";
	sun.setR(0.3);
	objects.push_back(sun);
	//1
	Object mercury;
	mercury.name = u8"水星"u8;
	mercury.location = glm::vec3(0.28f, 0.28f, 0.0f);
	mercury.texture_path = "textures/2k_mercury.jpg";
	mercury.setR(0.05);
	objects.push_back(mercury);
	//2
	Object venus;
	venus.name = u8"金星"u8;
	venus.location = glm::vec3(0.0f, 0.72f, 0.0f);
	venus.texture_path = "textures/2k_venus_atmosphere.jpg";
	venus.setR(0.05);
	objects.push_back(venus);
	//3
	Object earth;
	earth.name = u8"地球"u8;
	earth.location = glm::vec3(-0.71f, 0.71f, 0.0f);
	earth.texture_path = "textures/2k_earth_daymap.jpg";
	earth.setR(0.1);
	objects.push_back(earth);
	//4
	Object mars;
	mars.name = u8"火星"u8;
	mars.location = glm::vec3(-1.52f, 0.0f, 0.0f);
	mars.texture_path = "textures/2k_mars.jpg";
	mars.setR(0.1);
	objects.push_back(mars);
	//5
	Object jupiter;
	jupiter.name = u8"木星"u8;
	jupiter.location = glm::vec3(-3.68f, -3.68f, 0.0f);
	jupiter.texture_path = "textures/2k_jupiter.jpg";
	jupiter.setR(0.2);
	objects.push_back(jupiter);
	//6
	Object saturn;
	saturn.name = u8"土星"u8;
	saturn.location = glm::vec3(0.0f, -9.58f, 0.0f);
	saturn.texture_path = "textures/2k_saturn.jpg";
	saturn.setR(0.2);
	objects.push_back(saturn);
	//土星环占位符
	Object saturn_ring;
	saturn_ring.name = u8"土星"u8;
	saturn_ring.location = glm::vec3(0.0f, -9.58f, 0.0f);
	saturn_ring.texture_path = "textures/2k_saturn_ring_alpha.png";
	saturn_ring.setR(0.2);
	//7
	Object uranus;
	uranus.name = u8"天王星"u8;
	uranus.location = glm::vec3(13.59f, -13.59f, 0.0f);
	uranus.texture_path = "textures/2k_uranus.jpg";
	uranus.setR(0.2);
	objects.push_back(uranus);
	//8
	Object neptune;
	neptune.name = u8"海王星"u8;
	neptune.location = glm::vec3(30.05f, 0.0f, 0.0f);
	neptune.texture_path = "textures/2k_neptune.jpg";
	neptune.setR(0.2);
	objects.push_back(neptune);
	{
		glm::vec3 mercury(0.28f, 0.28f, 0.0f);
		glm::vec3 venus(0.0f, 0.72f, 0.0f);
		glm::vec3 earth(-0.71f, 0.71f, 0.0f);
		glm::vec3 mars(-1.52f, 0.0f, 0.0f);
		glm::vec3 jupiter(-3.68f, -3.68f, 0.0f);
		glm::vec3 saturn(0.0f, -9.58f, 0.0f);
		glm::vec3 uranus(13.59f, -13.59f, 0.0f);
		glm::vec3 neptune(30.05f, 0.0f, 0.0f);
	}
...
void updateObjects() {
	...
	//自转倍数 公转倍数
	const float rotationSpeedMultiplier=.5;
	const float orbitSpeedMultiplier=15;

	glm::mat4 rot = glm::rotate(glm::mat4(1.0f), totalTime * 2 * glm::pi<float>()* rotationSpeedMultiplier, glm::vec3(0.0f, 0.0f, 1.0f))
		* glm::rotate(glm::mat4(1.0f),  glm::pi<float>() /2, glm::vec3(1.0f, 0.0f, 0.0f));
	for (size_t i = 0; i < objects.size(); ++i) {
		if (i > 0) {
			objects[i].ratation = rot;
			float r = glm::length(objects[i].location); // 计算到原点的距离
			if (r == 0) return; // 避免除以0

			float T = 365.25 * pow(r,1.5); // 计算周期 天
			float deltaAngle = 2 * glm::pi<float>() / T * deltaTime* orbitSpeedMultiplier; // 计算旋转角度 /天

			// 创建旋转矩阵，绕z轴旋转
			glm::mat4 rotation = glm::rotate(glm::mat4(1.0f), deltaAngle, glm::vec3(0, 0, 1));

			// 更新位置
			glm::vec4 newPosition = rotation * glm::vec4(objects[i].location, 1.0f);
			objects[i].location = glm::vec3(newPosition);
		}
		objects[i].calculate();
	}
...
const std::vector<Vertex> vertices_simple = {
...
{{0.0f, -0.5f, 0.0f}, {0.0f, 1.0f, 1.0f}, {1.0f, 0.0f}},
};

const std::vector<uint32_t> indices_simple = {
			1, 0, 5, 2, 1, 5,
			3, 2, 5, 0, 3, 5,
			0, 1, 4, 1, 2, 4,
			2, 3, 4, 3, 0, 4,
};
const bool use_simple_graphics=false;
...
private:
	...
	void initWindow() {
		...
		appptr = this;
		window = glfwCreateWindow(WIDTH, HEIGHT, "Solor", nullptr, nullptr);
		...
		auto mouse_pos_callback = [](GLFWwindow* window, double xpos, double ypos) {
			...
			if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_RIGHT) == GLFW_PRESS) {
				...
				cameraptr->ProcessMouseMovement(offsetX, offsetY);
			}
			else {
		...
		auto mouse_click_callback = [](GLFWwindow* window, int button, int action, int mods) {
			...
			if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
				...
				xpos = xpos / appptr->swapChainExtent.width * 2.0 - 1.0;
				ypos= ypos / appptr->swapChainExtent.height * 2.0 - 1.0;
				glm::vec4 rayClip(xpos, ypos, -1,0);
				glm::mat4 view = cameraptr->GetViewMatrix();
				glm::mat4 proj = glm::perspective(glm::radians(cameraptr->Zoom), appptr->swapChainExtent.width / (float)appptr->swapChainExtent.height, 0.1f, 100.0f);
				proj[1][1] *= -1;
				glm::vec4 rayEye = glm::inverse(proj) * rayClip;
				rayEye = glm::vec4(rayEye.x, rayEye.y, -1.0, 0.0);
				glm::vec3 rayWorld = glm::normalize(glm::vec3(glm::inverse(view) * rayEye));
				glm::vec3 cameraPos=cameraptr->Position;
				std::cout << std::format("\n{}\t{}\t{}\n", rayWorld.x, rayWorld.y, rayWorld.z);
				float minDis=1e6;
				int index = 0;
		
				for (Object& object : objects) {
					glm::vec3 AP = object.location-cameraPos;
					float disAP = glm::length(AP);
					//P为物体所在点 C为垂线交点
					glm::vec3 CP = glm::cross(AP, rayWorld);
					float disCP = glm::length(CP);
					float disAC = sqrt(disAP * disAP - disCP * disCP);
					//当射线穿过球(即CP小于半径)时 射线到交点最短的即为选中物体
					if (disCP < object.r && disAC< minDis) {
						minDis = disAC;
						select_index = index;
					}
					index++;
	...
	void initVulkan() {
		...
		//复数贴图
		for (Object& object : objects) {
			UploadTexture(object.texture_path);
		}
		createDescriptorPool_imgui();
	...
	void initGui() {
		...
		init_info.QueueFamily = this->graphicsQueueIndex;
		init_info.Queue = this->graphicsQueue;
		init_info.PipelineCache = VK_NULL_HANDLE;
		init_info.DescriptorPool = this->descriptorPool_imgui;
		init_info.Subpass = 1;
		init_info.MinImageCount = this->swapChainImages.size();
	...
	void mainLoop() {
		while (!glfwWindowShouldClose(window)) {
			...
			ImGui::NewFrame();
			{
				...
				ImGui::Begin(reinterpret_cast< const char*>(u8"星球信息"));                          // Create a window called "Hello, world!" and append into it.
				if (i >= 0) {
	...
	void drawFrame() {
		//帧率控制
		static auto next = std::chrono::high_resolution_clock::now();
		auto currentTime = std::chrono::high_resolution_clock::now();
		const int frameratelock = 170;
		while (frameratelock>0&&currentTime < next) {
			currentTime = std::chrono::high_resolution_clock::now();
		}
		next = currentTime+ std::chrono::microseconds((long long)(1e6 / frameratelock));
		...
		updateDynamicUniformBuffer(currentFrame);
	...
	{
		updateObjects();//更新object的位置
		uint32_t index = 0;
		for (Object& object : objects) {
			DynamicBufferObject* objectData = (DynamicBufferObject*)(((uint64_t)uboDynamic.data + (index * dynamicAlignment)));
			objectData->model = object.model;
			++index;
		}

		void* data = reinterpret_cast<size_t*>(dynamicUniformData[currentImage]) + normalUBOAlignment / sizeof(size_t);
		memcpy(data, uboDynamic.data, objects.size() * dynamicAlignment);
	...
	void createSwapChain() {
		...
		createInfo.compositeAlpha = VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR;//混合
		createInfo.presentMode = presentMode;
		createInfo.clipped = VK_TRUE;//被遮挡时允许优化
	...
	VkPresentModeKHR chooseSwapPresentMode(const std::vector<VkPresentModeKHR> availablePresentModes) {
		VkPresentModeKHR bestMode = VK_PRESENT_MODE_FIFO_KHR;//3
		for (const auto& availablePresentMode : availablePresentModes) {
			if (availablePresentMode == VK_PRESENT_MODE_MAILBOX_KHR)//1
			{
				return availablePresentMode;
			}
			else if (availablePresentMode == VK_PRESENT_MODE_IMMEDIATE_KHR) {//2
	...
	void createImageViews() {
		...
		for (uint32_t i = 0; i < swapChainImages.size(); i++) {
			swapChainImageViews[i] = createImageView(swapChainImages[i], swapChainImageFormat, VK_IMAGE_ASPECT_DEPTH_BIT, 1);
		}
	}
	void createRenderPass() {
		...
		attachment_color.format = swapChainImageFormat;
		attachment_color.samples = msaaSamples;
		...
		attachment_color.finalLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
		attachments.push_back(attachment_color);
		//[1]
		VkAttachmentDescription attachment_depth = {};
		attachment_depth.format = findDepthFormat();
		attachment_depth.samples = msaaSamples;
		...
		attachments.push_back(attachment_depth);
		//[2]
		VkAttachmentDescription attachment_resolve{};
		attachment_resolve.format = swapChainImageFormat;
		...
		attachment_resolve.finalLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
		attachments.push_back(attachment_resolve);
		//[3] ui color
		VkAttachmentDescription attachment_color_ui = {};
		attachment_color_ui.format = swapChainImageFormat;
		attachment_color_ui.samples = VK_SAMPLE_COUNT_1_BIT;
		attachment_color_ui.loadOp = VK_ATTACHMENT_LOAD_OP_LOAD;
		attachment_color_ui.storeOp = VK_ATTACHMENT_STORE_OP_STORE;
		attachment_color_ui.stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
		attachment_color_ui.stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
		attachment_color_ui.initialLayout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
		attachment_color_ui.finalLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
		//attachments.push_back(attachment_color_ui);
		//子流程和附着引用
		//1>渲染
		...
		VkAttachmentReference attachRef_resolve{};
		attachRef_resolve.attachment = 2;
		attachRef_resolve.layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
		//2>UI
		VkAttachmentReference attachRef_color_ui = {};
		attachRef_color_ui.attachment = 0;
		attachRef_color_ui.layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
		VkAttachmentReference attachRef_depth_ui = {};
		attachRef_depth_ui.attachment = 1;
		attachRef_depth_ui.layout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;


		//多subpass
		std::vector<VkSubpassDescription>subpasses;
		//subpass 渲染
		VkSubpassDescription subpass_render = {};
		subpass_render.pipelineBindPoint = VK_PIPELINE_BIND_POINT_GRAPHICS;
		subpass_render.colorAttachmentCount = 1;
		subpass_render.pColorAttachments = &attachRef_color;
		subpass_render.pDepthStencilAttachment = &attachRef_depth;
		subpass_render.pResolveAttachments = &attachRef_resolve;
		subpasses.push_back(subpass_render);
		//subpass UI
		...
		subpass_imgui.pColorAttachments = &attachRef_color_ui;
		subpass_imgui.pDepthStencilAttachment = &attachRef_depth_ui;
		subpass_imgui.pResolveAttachments = &attachRef_resolve;
		subpasses.push_back(subpass_imgui);
		//
		std::vector < VkSubpassDependency> dependencies;
		//1>
		VkSubpassDependency dependency_render = {};
		dependency_render.srcSubpass = VK_SUBPASS_EXTERNAL;
		dependency_render.dstSubpass = 0;
		dependency_render.srcStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
		dependency_render.srcAccessMask = 0;
		dependency_render.dstStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
		dependency_render.dstAccessMask = VK_ACCESS_COLOR_ATTACHMENT_READ_BIT | VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
		dependencies.push_back(dependency_render);
		//2>
		VkSubpassDependency dependency_imgui = {};
		dependency_imgui.srcSubpass = 0;
		dependency_imgui.dstSubpass = 1;
		...
		dependency_imgui.dstAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
	...
	void createGraphicsPipeline() {
		VkShaderModule vertShaderModule = createShaderModule(device, "shaders", "shader.vert", "vert.spv");
		VkShaderModule fragShaderModule = createShaderModule(device, "shaders", "shader.frag", "frag.spv");
		...
		VkPipelineViewportStateCreateInfo viewportState = {};//视口和裁剪组合
	...
	void createTextureImageView() {
		textureImageView = createImageView(textureImage, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_DEPTH_BIT, mipLevels);
	}
	...
	VkSampler* createTextureSampler(uint32_t mipLevels=0) {
		if (mipLevels == 0)mipLevels = this->mipLevels;
	...
	void createVertexBuffer() {
		...
		vkUnmapMemory(device, stagingBufferMemory);//结束映射
	...
	void createUniformBuffer() {
		//dynamic
		static int UNIFORM_INSTANCE_COUNT = 200;
		VkPhysicalDeviceProperties properties;
		vkGetPhysicalDeviceProperties(physicalDevice, &properties);
		size_t minUboAlignment = properties.limits.minUniformBufferOffsetAlignment;
		dynamicAlignment = sizeof(DynamicBufferObject);
		if (minUboAlignment > 0) {
			dynamicAlignment = (dynamicAlignment + minUboAlignment - 1) & ~(minUboAlignment - 1);
		}
		VkDeviceSize bufferSize = UNIFORM_INSTANCE_COUNT * dynamicAlignment;
		uboDynamic.data = (DynamicBufferObject*)_aligned_malloc(bufferSize, dynamicAlignment);
		...
		for (size_t i = 0; i < swapChainImages.size(); i++) {
			...
			vkMapMemory(device, uniformBuffersMemory[i], 0, UNIFORM_INSTANCE_COUNT * dynamicAlignment, 0, &dynamicUniformData[i]);
		}
	}
	...
	void createDescriptorPool_imgui() {
		...
		if (vkCreateDescriptorPool(device, &poolInfo, nullptr, &descriptorPool_imgui) != VK_SUCCESS) {
			throw std::runtime_error("failed to create descriptor pool!");
		}
	}
	...
	void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
		...
		vkCmdBeginRenderPass(commandBuffer, &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
		{
			...
			for (uint32_t j = 0; j < objects.size(); ++j)
			{
				...
				VkDescriptorSet sets[2] = { descriptorSets[currentFrame] , textureInfos[j].descriptorSet };
				vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 2, sets, 1, &dynamicOffset);
