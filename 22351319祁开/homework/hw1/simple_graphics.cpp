...
const bool use_simple_graphics=true;
...
private:
	...
	void initWindow() {
		...
		window = glfwCreateWindow(WIDTH, HEIGHT, "Simple Graphics", nullptr, nullptr);
	...
	void drawFrame() {
		...
		const int frameratelock = 30;
		while (frameratelock>0&&std::chrono::duration_cast<std::chrono::microseconds>(currentTime - lastTime).count()<(1e6/ frameratelock)) {
	...
	void updateDynamicUniformBuffer(uint32_t currentImage)
	{
		...
		objects[0].model = glm::translate(glm::mat4(1.0f), glm::vec3(1., 0, 0)) * model;
		objects[1].model = glm::translate(glm::mat4(1.0f), glm::vec3(-1., 0, 0)) * model;
		uint32_t index = 0;
		for (auto& object : objects) {
			glm::mat4* modelMat = (glm::mat4*)(((uint64_t)uboDynamic.model + (index * dynamicAlignment)));
			*modelMat = object.model;
			++index;
		}

		void* data = reinterpret_cast<size_t*>(dynamicUniformData[currentImage]) + normalUBOAlignment / sizeof(size_t);
		memcpy(data, uboDynamic.model, objects.size() * dynamicAlignment);
	...
	void createUniformBuffer() {
		...
		uboDynamic.model = (glm::mat4*)_aligned_malloc(bufferSize, dynamicAlignment);
