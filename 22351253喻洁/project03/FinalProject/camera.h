...
public:
    ...
    void ProcessKeyboard(Camera_Movement direction, float deltaTime, GLboolean constrainSpeed = true)
    {
        ...
        if (direction == RIGHT)
            Position += Right * velocity;
        // UP and DOWN movement to be used with keys Q and E
        if (direction == UP)
            Position += Up * velocity;
        if (direction == DOWN)
            Position -= Up * velocity;
        
        // MovementSpeed capped at 20 as it seems fast but not unwieldly.
        // Makes sure that movement speed does not become 0 (halts all movement), or less than 0 (Flips movement axis).
        if (constrainSpeed)
        {
    ...
    void ProcessMouseMovement(float xoffset, float yoffset, GLboolean constrainPitch = true, GLboolean constrainSens = true)
    {
