...
int main(int argc, char **argv)
{
    ...
    // setup camera
    CameraManip.setLookat({.0f, .0f, -5.f}, {.0f, .0f, .0f}, {.0f, 1.f, .0f});
    CameraManip.setClipPlanes({.01f, 100.f});
    ...
    app.setup(context.m_instance, context.m_physicalDevice, context.m_device, context.m_queueGCT);
    ...
    while (glfwWindowShouldClose(window) == GLFW_FALSE)
    {
        ...
        // rendering in swapchain framebuffer
        {
