...
std::string showName;
std::vector<std::string> planetNames = { "sun","mercury","venus","earth","mars","jupiter","saturn","uranus","neptune","pluto"};
...
void init(void)
{
	...
	solarSystem.addPlanet("sun",0, 1, 500, 695500, sun->getTextureHandle()); // sun
	solarSystem.addPlanet("mercury",57910000, 88, 58.6, 2440, mercury->getTextureHandle()); // mercury
	solarSystem.addPlanet("venus",108200000, 224.65, 243, 6052, venus->getTextureHandle()); // venus
	solarSystem.addPlanet("earth", 149600000, 365, 1, 6371, earth->getTextureHandle()); // earth
	solarSystem.addPlanet("mars", 227939100, 686, 1.03f, 3389, mars->getTextureHandle()); // mars
	solarSystem.addPlanet("jupiter", 778500000, 4332, 0.4139, 69911, jupiter->getTextureHandle()); // jupiter
	solarSystem.addPlanet("saturn", 1433000000, 10759, 0.44375, 58232, saturn->getTextureHandle()); // saturn
	solarSystem.addPlanet("uranus", 2877000000, 30685, 0.718056, 25362, uranus->getTextureHandle()); // uranus
	solarSystem.addPlanet("neptune", 4503000000, 60188, 0.6713, 24622, neptune->getTextureHandle()); // neptune
	solarSystem.addPlanet("pluto", 5906380000, 90616, 6.39, 1137, pluto->getTextureHandle()); // pluto
...
void display(void)
{
	...
	glRasterPos2i(10, 20);            //Top left corner of text
	std::string t1 = "Press 1-9 to show the plannet, WASD or IJKL to move";
	...
	glColor4f(1.0f, 1.0f, 1.0f, 1.0f);  //RGBA values of text color
	glRasterPos2i(10, 40);            //Top left corner of text
	std::string t2 = "Now look at:"+ showName;
	// Since 2nd argument of glutBitmapString must be const unsigned char*
	for (char c : t2)
	{
		glutBitmapCharacter(GLUT_BITMAP_HELVETICA_12, c);
