...
Planet::Planet(std::string Name, float distanceFromSun, float orbitTime, float rotationTime, float radius, GLuint textureHandle)
{
	this->Name = Name;
...
void Planet::renderText(float x, float y, float z, const std::string& text)
{
	...
	// 使用 OpenGL 的 glutBitmapString 函数渲染文本
	for (char c : text)
	{
