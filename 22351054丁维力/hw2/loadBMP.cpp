...
texture * sun, *earth, *moon,*sunName;			//纹理指针
...
BOOL LoadAllTextures() {
	...
	sunName = LoadTexFile("font1.bmp");
	if (sunName == NULL)
		return FALSE;

	glBindTexture(GL_TEXTURE_2D, sunName->texID);
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
	gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGBA, sunName->width, sunName->height, GL_RGB, GL_UNSIGNED_BYTE, sunName->data);
