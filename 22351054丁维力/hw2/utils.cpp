...
void Utils::CastRay(int x, int y)
{
	GLint    viewport[4];
	...
	float t1 = IntersectSphere(O, D, glm::vec3(0.0, 0.0, 0.0), 0.20) ;
	float t2 = IntersectSphere(O, D, earthpos, 0.1);
	float t = min(t1, t2);
	if (t < INF)
	{
		showTimes = 1000;
		if (t1 < t2)
		{
			_obj = ClickedObject::Sun;
		}
		else {
			_obj = ClickedObject:: Earth;
		}
	}
	else {
...
float Utils::IntersectSphere(glm::vec3 O, glm::vec3 D, glm::vec3 S, float R)
{
	glm::vec3 OS = O - S;
