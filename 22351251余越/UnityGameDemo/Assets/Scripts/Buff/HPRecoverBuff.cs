...
public class HPRecoverBuff : Buff
{
    void Update()
    {
    ...
    public void launch()
    {
        gameObject.GetComponent<CharacterStats>().currentHealthRecover = (int)(gameObject.GetComponent<CharacterStats>().baseHealthRecover + PlusNumAll);
        ...
        for (int i = 0; i < targetBuffPanel.transform.childCount; i++)
        {
            ...
            if (buffGO.GetComponent<BuffIcon>().buffType == BuffType.HPRECOVER)
            {
